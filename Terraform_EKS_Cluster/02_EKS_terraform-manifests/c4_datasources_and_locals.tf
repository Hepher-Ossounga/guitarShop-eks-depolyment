...
locals {
  ...
  owners = var.business_division  # Example: "guitarshop"
  ...
  name = "${local.owners}-${local.environment}"  # Example: "guitarshop-dev"

  # Full EKS cluster name used for resource naming and tagging
  eks_cluster_name = "${local.name}-${var.cluster_name}"  # Example: "guitarshop-dev-guitarshop-eks"
}
